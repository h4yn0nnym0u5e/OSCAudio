...
Use **OSCSubscription(void (\*routeFunction)(OSCMessage\*,OSCBundle&)** to instantiate a new subscription object, The required **routeFunction** parameter will be called whenever a subscription fires: the OSCMessage parameter will be any OSC message that has been "subscribed", and the OSCBundle is used to collate any responses.
...
This routing function forwards a [partially used] message to the OSCSubscription router, which currently implements two methods:
