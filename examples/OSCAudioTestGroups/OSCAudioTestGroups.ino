...
void listObjects(OSCAudioBase* obj,int indent=0)
{
  ...
  while (depth < 5 && NULL != obj)
  {
    ...
    for (int i=0;i<indent;i++)
      Serial.print(' ');
    Serial.printf("%s is at %08X; route: %08X",obj->name,(uint32_t) obj,(uint32_t) obj->getNext());
    if (NULL != grp)
    {
...
void setup() {
  ...
  OSCAudioGroup* voice1 = new OSCAudioGroup("voice1");
    ...
    OSCAudioGroup* v1i0 = new OSCAudioGroup("i0",voice1);
      ...
      OSCAudioMixer4* mixer2 = new OSCAudioMixer4("mixer2",*v1i0);
      ...
      OSCAudioMixer4* mixer2b = new OSCAudioMixer4("mixer2",*v1i1);
